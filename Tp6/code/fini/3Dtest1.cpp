...
Point* Cone(Point S, double rayon, double hauteur, long n);
...
void DrawCylindre(Point* P,Point*Q, int taille){
	...
	for(int i=0;i<taille;i++){
			glVertex3f(P[i].getX(),P[i].getY(),P[i].getZ());
			glVertex3f(Q[i].getX(),Q[i].getY(),Q[i].getZ());
			glVertex3f(Q[i+1].getX(),Q[i+1].getY(),Q[i+1].getZ());
...
Point* Cone(Point S, double rayon, double hauteur, long n){
	...
	for(int k =0;k<n;k++){
		...
		double z= 1;//sqrt(zs*zs/(rayon*rayon*(x+y)*(x+y)))+zs;
		cout<<"  k  "<<k<<" x  "<<x<<"  y "<<y<<endl;

		Pb[k].setX(x);
		Pb[k].setY(y);
		Pb[k].setZ(z);
		//cout<<"  k  "<<k<<" x  "<<Pb[k].getX()<<"  y "<<Pb[k].getY()<<"  z  "<<Pb[k].getZ()<<endl;
		//if(k==0)
			Pb[k].AfficherPoint();
...
Point** Sphere(double rayon, long meridien,long parallele){
	Point **P=NULL;
	if(meridien>=3 && parallele>=2){
		double teta=M_PI/(double)meridien;
		double Phi=2*M_PI/(double)parallele;
		...
		z=rcos(phi)
		*/
		int taille=(int)meridien*parallele;
		P=new Point*[(int)meridien];
		for(int i=0;i<meridien;i++)
			P[i]=new Point[(int)parallele];
		...
		for(int i=0;i<meridien;i++)
		{
			for(int k=0;k<parallele;k++){
				...
				//cout<<"i   "<<i<<"  k  "<<k<<" x  "<<P[i][k].getX()<<"  y "<<P[i][k].getY()<<"  z  "<<P[i][k].getZ()<<endl;
